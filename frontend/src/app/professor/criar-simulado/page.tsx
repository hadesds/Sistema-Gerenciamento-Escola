...
function difBadge(d: string, label: string) {
  const bg = d === 'facil' ? 'var(--color-success)' : d === 'dificil' ? 'var(--color-danger)' : 'var(--color-warning)';
  return <span className="badge" style={{ background: bg, color: '#fff', fontSize: '1.2rem' }}>{label}</span>;
}
...
export default function CriarSimuladoPage() {
  ...
  function toggleQuestao(id: number) {
    setQuestoesSelecionadas(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  }

  function selecionarTodosFiltrados() {
    const ids = questoesFiltradas.map(q => q.id);
    setQuestoesSelecionadas(prev => Array.from(new Set([...prev, ...ids])));
  }

  function desselecionarTodosFiltrados() {
    const ids = new Set(questoesFiltradas.map(q => q.id));
    setQuestoesSelecionadas(prev => prev.filter(id => !ids.has(id)));
  }

  const todosFiltradosSelecionados = questoesFiltradas.length > 0 && questoesFiltradas.every(q => questoesSelecionadas.includes(q.id));
  ...
  return (
    <ProtectedRoute tipo="professor">
      ...
      <main className="container fade-in">
        <style>{`
          /* Área ENEM chips para configuração */
          ...
          /* Filtro chips (matéria) */
          .filtro-chips { display: flex; flex-wrap: wrap; gap: 0.8rem; }

          /* Toggle tempo (idêntico ao check-row do banco-questoes) */
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="card mb-2">
              ...
              <div className="form-group">
                <label>Título do simulado <span style={{ color: '#94a3b8', fontWeight: 400 }}>(opcional)</span></label>
                <input
                  type="text"
                  value={titulo}
              ...
              <div className="form-group">
                <label>Turma alvo <span style={{ color: 'var(--color-danger)' }}>*</span></label>
              ...
              <div className="form-group">
                ...
                >
                  <div className="check-box">
                    {usarTempo && <span className="material-icons-outlined">check</span>}
                  </div>
                  <span style={{ fontSize: '1.45rem', fontWeight: 600 }}>Definir tempo limite para o simulado</span>
                </div>
                {usarTempo && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '1rem' }}>
                    <input
                      ...
                      style={{ width: '10rem' }}
                    />
                    <span style={{ fontSize: '1.45rem', color: 'var(--text-secondary)' }}>minutos</span>
                  </div>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                <div className="area-chips">
                  <button
                    ...
                    style={areaSimulado === '' ? { background: '#1a73c7', borderColor: 'transparent', color: '#fff' } : {}}
                  ...
                  {AREAS_ENEM.map(area => (
                    <button
                      ...
                      style={areaSimulado === area.label ? { background: area.cor, borderColor: 'transparent', color: '#fff' } : { borderColor: area.cor + '55' }}
                      onClick={() => {
                        setAreaSimulado(area.label);
                        setAreaFiltro(area.label);
                      }}
                    >
                      <span className="material-icons-outlined" style={{ color: areaSimulado === area.label ? '#fff' : area.cor }}>{area.icon}</span>
                      {area.label}
                    </button>
                  ))}
                </div>
            ...
            <div className="card mb-2">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.6rem' }}>
                <h2>2. Questões</h2>
                <span className="badge" style={{ background: '#1a73c7', color: '#fff', fontSize: '1.3rem' }}>
              ...
              <div className="filtro-secao">
                ...
                <div className="area-chips">
                  <button
                    ...
                    style={areaFiltro === '' ? { background: '#1a73c7', borderColor: 'transparent', color: '#fff' } : {}}
                  ...
                  {AREAS_ENEM.map(area => (
                    <button
                      ...
                      style={areaFiltro === area.label ? { background: area.cor, borderColor: 'transparent', color: '#fff' } : { borderColor: area.cor + '55' }}
                      onClick={() => setAreaFiltro(area.label)}
                    >
                      <span className="material-icons-outlined" style={{ color: areaFiltro === area.label ? '#fff' : area.cor }}>{area.icon}</span>
                      {area.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="filtro-secao">
                <div className="filtro-secao-label">Filtrar por data de criação</div>
              ...
              <div className="sel-actions">
                <span>{questoesFiltradas.length} questão(ões) nos filtros atuais</span>
                <div style={{ display: 'flex', gap: '0.8rem', flexWrap: 'wrap' }}>
                  <button
                    ...
                    disabled={todosFiltradosSelecionados || questoesFiltradas.length === 0}
                  >
              ...
              {questoesFiltradas.length === 0 ? (
                ...
                questoesFiltradas.map(q => (
                  <label key={q.id} className={`questao-item${questoesSelecionadas.includes(q.id) ? ' selecionada' : ''}`}>
                    <input
                      type="checkbox"
